        ...
        if (doc.firstChild && doc.firstChild.nodeName === "html") {
            ...
            if (options.failure) {
                options.failure.call(
                    options.scope,
                    "Error performing WPS Execute through form POST, status: " + statusText,
                    body
                );
            }
        } else {
...
wps.process.prototype.describe = function(options) {
    ...
    if (!this.description) {
        this.client.describeProcess(
            this.server,
            this.identifier,
            function(description, error, statusText) {
                ...
                if (desc.processOutputs) {
                    for (var i = 0, ii = desc.processOutputs.output.length; i < ii; ++i) {
                        ...
                        if (
                            ...
                            for (
                                var j = 0, jj = output.complexOutput.supported.format.length;
                                j < jj;
                                ++j
                            ) {
                                if (
                                    output.complexOutput.supported.format[j].mimeType ===
                                    "image/tiff"
                                ) {
                                    hasTiffOutput = true;
                                    break;
                ...
                if (hasTiffOutput) {
                    ...
                    xmlhttp.onerror = function() {
                        if (options.failure) {
                            options.failure.call(
                                options.scope,
                                "Network error on WPS:Execute request",
                                body
                            );
                        }
                    };
                    xmlhttp.onload = function() {
                        if (this.status !== 200) {
                            if (options.failure) {
                                options.failure.call(
                                    options.scope,
                                    "HTTP status error on WPS:Execute request: " + this.statusText,
                                    body
                                );
                                return;
                            }
                        ...
                        if (this.responseText.indexOf("ExceptionText") !== -1) {
                            var info = me.client.unmarshaller.unmarshalDocument(this.responseXML)
                                .value;
                            var exception = wps.client.getExceptionText(info);
                            if (options.failure) {
                                options.failure.call(options.scope, exception, body);
                        ...
                        if (output.literalOutput) {
                            ...
                            var box = me.client.unmarshaller.unmarshalDocument(this.responseXML)
                                .value;
                            ...
                            if (output.complexOutput._default.format.mimeType === "text/xml") {
                                ...
                                for (var i = 0, ii = me.formats.length; i < ii; ++i) {
                                    if (me.formats[i].mimeType === mimeType) {
                                        try {
                                            result = me.formats[i].format.readFeatures(
                                                this.responseText
                                            );
                                        } catch (e) {
                                            if (window.console) {
...
wps.process.prototype.parseDescription = function(description) {
    ...
    this.description = this.client.unmarshaller.unmarshalString(
        server.processDescription[this.identifier]
    ).value.processDescription[0];
};
    ...
    if (data instanceof wps.process.chainlink) {
        ...
        inputValue = {
            ...
            reference: {
                method: "POST",
                href:
                    data.process.server === this.server
                        ? this.localWPS
                    ...
                    if (this.formats[i].mimeType === format) {
                        if (typeof data === "string") {
                            if (format === "application/wkt") {
                                ...
                                content = this.formats[i].format.writeFeatures(
                                    this.WKT.readFeatures(data)
                                );
...
wps.process.prototype.chainProcess = function(input, inputValue, chainLink) {
    ...
    inputValue.reference.mimeType = input.complexData
        ? this.findMimeType(
              input.complexData.supported.format,
              chainLink.process.description.processOutputs.output[output].complexOutput.supported
                  .format
          )
        : input.literalData.dataType.value;
    var formats = {};
...
wps.client = function(options) {
    ...
    for (var s in options.servers) {
        this.servers[s] =
            typeof options.servers[s] == "string"
                ? {
...
wps.client.getExceptionText = function(info) {
    ...
    if (
        info["status"] &&
        info["status"].processFailed &&
        info["status"].processFailed.exceptionReport &&
        info["status"].processFailed.exceptionReport.exception
    ) {
        for (
            i = 0, ii = info["status"].processFailed.exceptionReport.exception.length;
            i < ii;
            ++i
        ) {
            exception += info["status"].processFailed.exceptionReport.exception[
                i
            ].exceptionText.join("<br/>");
        }
...
wps.client.prototype.getFeatureTypes = function(serverID, callback) {
    ...
    var xmlhttp = new XMLHttpRequest();
    server.url.replace("/ows", "/search/content");
...
wps.client.prototype.getGroupedProcesses = function(serverID, callback) {
    ...
    var me = this;
    var errorText =
        "There was an error loading the WPS GetCapabilities document from: " +
        server.url +
